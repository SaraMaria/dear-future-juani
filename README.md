...
A tela "🎧 Playlist" mostra a playlist do Spotify incorporada direto na página, via `iframe` oficial do Spotify (sem precisar de chave de API nem login). O `iframe` é criado por JavaScript só no momento em que a tela é aberta (e não direto no HTML) — isso evita um problema comum em sites com várias "telas" escondidas, onde alguns navegadores nunca carregam um `iframe` que nasce dentro de um elemento com `display:none`. Como rede de segurança, também tem um link "▶ Abrir la playlist en Spotify ↗" logo abaixo do player, que funciona mesmo se o `iframe` for bloqueado por algum bloqueador de anúncios/rastreadores.

Para trocar de playlist, edite duas coisas no `script.js`, dentro da função `renderPlaylist()`: a URL do `iframe.src` (troque o ID depois de `/embed/playlist/`) e, no `index.html`, o link `href` do fallback "Abrir en Spotify".
